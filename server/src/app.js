...
app.post('/api/stream-analysis', async (req, res) => {
  try {
    const { analysis, userQuestion } = req.body;
    ...
    // Regular response headers for axios streaming
    res.setHeader('Content-Type', 'text/plain');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');

    const systemPrompt = `You are a data analysis expert. Explain the following analysis output from python code for user request in a clear, 
    conversational way in a concise manner. Return in markdown format. `;
    
    const userPrompt = `The user asked: "${userQuestion}"
    
    Here is the JSON data that was generated by the python code for help in user query analysis:
    ${formattedAnalysis}
    
    Explain what this means in a clear and concise way and consider using tabular.`;
